...
export const Chat: React.FC<ChatProps> = ({ user }) => {
  ...
  const getErrorMessage = (error: any): string => {
    ...
    if (status === 429 || msg.includes('429')) return "Neural processors are overloaded (Rate Limit). Please stand by.";
    if (status >= 500) return "The Central Intelligence Core is temporarily offline. Please retry.";
    if (msg.includes('SAFETY')) return "Safety Protocol Violation: Your request was flagged and intercepted.";
    if (msg.includes('API_KEY')) return "Authentication Failure: API Access Key error.";
    if (msg.includes('FETCH') || msg.includes('NETWORK')) return "Uplink Severed: Check your connection.";
    
    return "Critical System Fault: An unexpected error occurred. Retry shortly.";
  };
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const errorMsg: ChatMessage = {
        id: (Date.now() + 1).toString(),
        role: 'model',
        text: `⚠️ **SYSTEM ALERT**: ${getErrorMessage(error)}`,
        timestamp: Date.now()
      };
      setMessages(prev => [...prev, errorMsg]);
      if (error?.message?.toUpperCase().includes('SESSION')) {
          ...
          return (
            <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[85%] flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                ...
                <div className={`p-3 rounded-2xl text-sm leading-relaxed ${msg.role === 'user' ? 'bg-titan-gray text-white rounded-tr-none' : isError ? 'bg-titan-danger/10 border border-titan-danger/50 text-titan-danger rounded-tl-none' : 'bg-titan-dark border border-titan-gray/50 text-gray-200 rounded-tl-none'}`}>
